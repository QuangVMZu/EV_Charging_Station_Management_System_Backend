...
public class SlotAvailabilityServiceImpl implements SlotAvailabilityService {
    ...
    @Override
    @Transactional
    public int deleteByTemplate_Config_ConfigIdAndDateBetween(Long configId,
                                                              LocalDateTime start,
                                                              LocalDateTime end) {
        // 1) Xóa booking_slots trước (nếu có)
//        int deletedBookings = bookingSlotRepository.deleteByConfigIdAndDateRange(configId, start, end);

        // 2) Sau đó mới xóa slot_availability
        int deletedSlots = slotAvailabilityRepository.deleteByConfigIdAndDateRange(configId, start, end);
