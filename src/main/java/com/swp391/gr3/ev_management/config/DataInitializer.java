...
//        attachViolationToTriplet(driver, v);
    ...
    private void initReports() {
        try {
            // Nếu đã có dữ liệu rồi thì bỏ qua cho đỡ trùng
//            if (reportRepository.count() > 0) {
//                log.info("Reports already exist, skip seeding incidents.");
//                return;
//            }

            List<ChargingStation> stations = chargingStationRepository.findAll();
            if (stations.isEmpty()) {
                log.warn("No stations found, skip report seeding.");
                return;
            }

            List<Staffs> allStaffs = staffsRepository.findAll();

            // Mỗi station 3 report => ~30 report với 10 station
            for (ChargingStation station : stations) {
                Staffs staff = findAnyActiveStaffForStation(station, allStaffs);

                if (staff == null) {
                    log.warn("No active staff assigned to station {}, skip report seeding for this station",
                            station.getStationName());
                    continue;
                }

                // 3 report với 3 cấp độ
                createReportIfNotExists(
                        station,
                        staff,
                        "[MINOR] Kiểm tra đầu sạc định kỳ tại " + station.getStationName(),
                        "Nhân viên phát hiện cần kiểm tra bảo trì nhẹ cho một số cổng sạc.",
                        "medium",
                ...
                createReportIfNotExists(
                        station,
                        staff,
                        "[MAJOR] Sự cố kết nối mạng tại " + station.getStationName(),
                        "Hệ thống quản lý trạm bị gián đoạn kết nối trong khoảng 15 phút, cần kiểm tra router & switch.",
                        "high",
                        ReportStatus.REPORTED,
                ...
                createReportIfNotExists(
                        station,
                        staff,
                        "[CRITICAL] Cổng sạc không phản hồi tại " + station.getStationName(),
                        "Một cổng sạc DC công suất cao không phản hồi, đã tạm khóa cổng và chờ kỹ thuật xử lý.",
                        "low",
                        ReportStatus.REPORTED,
            ...
            log.info("✅ Seeded sample incidents/reports for stations.");

        } catch (Exception e) {
            log.error("❌ Failed to seed incidents/reports: {}", e.getMessage(), e);
        }
    }

    /**
     * Tìm một staff đang ACTIVE và đang được assign vào station (StationStaff.unassignedAt == null).
     * Nếu không có thì trả về null.
     */
    private Staffs findAnyActiveStaffForStation(ChargingStation station, List<Staffs> allStaffs) {
        for (Staffs s : allStaffs) {
            if (s.getStatus() != StaffStatus.ACTIVE) continue;
            if (s.getStationStaffs() == null) continue;

            boolean assignedToThisStation = s.getStationStaffs().stream()
                    .anyMatch(ss -> ss.getStation() != null
                            && ss.getStation().getStationId().equals(station.getStationId())
                            && ss.getUnassignedAt() == null);
    ...
    /**
     * Tạo report mẫu nếu chưa tồn tại một report cùng title tại cùng station.
     */
    private void createReportIfNotExists(ChargingStation station,
                                         Staffs staff,
                                         String title,
                                         String description,
            /* ReportSeverity */ Object severity, // TODO: đổi kiểu thành enum thật của bạn
                                         ReportStatus status,
                                         LocalDateTime reportedAt) {
        try {
            // Ở đây giả sử bạn có method existsByStation_StationIdAndTitleIgnoreCase,
            // nếu chưa có thì có thể bỏ check này hoặc tự định nghĩa trong ReportRepository.
            boolean exists = reportRepository
                    .existsByStation_StationIdAndTitleIgnoreCase(station.getStationId(), title);

            if (exists) {
                log.info("Report already exists for station {} with title '{}'", station.getStationName(), title);
                return;
            }
            ...
            // TODO: sau khi bạn xác định enum severity, set lại:
            // r.setSeverity((ReportSeverity) severity);

            r.setStatus(status != null ? status : ReportStatus.REPORTED);
            r.setReportedAt(reportedAt != null ? reportedAt : LocalDateTime.now());

            reportRepository.save(r);
            log.info("Created report '{}' for station {}", title, station.getStationName());
        } catch (Exception e) {
            log.warn("Failed to create report '{}' for station {}: {}",
                    title, station.getStationName(), e.getMessage());
        }
    }
}
